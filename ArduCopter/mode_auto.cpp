...
bool ModeAuto::wp_start(const Location& dest_loc)
{
    ...
    if (!wp_nav->is_active()) {
        ...
        if (_mode == SubMode::TAKEOFF) {
            ...
            if (auto_takeoff.get_completion_pos(takeoff_complete_pos_neu_m)) {
...
void ModeAuto::do_loiter_to_alt(const AP_Mission::Mission_Command& cmd)
{
    ...
    // set vertical speed and acceleration limits
    pos_control->set_max_speed_accel_U_m(wp_nav->get_default_speed_down_cms(), wp_nav->get_default_speed_up_cms(), wp_nav->get_accel_U_cmss());
    pos_control->set_correction_speed_accel_U_m(wp_nav->get_default_speed_down_cms(), wp_nav->get_default_speed_up_cms(), wp_nav->get_accel_U_cmss());
