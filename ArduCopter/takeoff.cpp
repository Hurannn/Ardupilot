...
{
    copter.flightmode->takeoff.start(takeoff_alt_m);
    return true;
}
...
void Mode::_TakeOff::start(float alt_m)
{
...
void _AutoTakeoff::run()
{
    ...
    if (is_terrain_alt && !wp_nav->get_terrain_offset_m(terr_offset_u_m)) {
    ...
    float pos_u_m = complete_alt_m + terr_offset_u_m;
    float vel_u_ms = 0.0;
    copter.pos_control->input_pos_vel_accel_U_m(pos_u_m, vel_u_ms, 0.0);
...
void _AutoTakeoff::start(float _complete_alt_m, bool _is_terrain_alt)
{
...
bool _AutoTakeoff::get_completion_pos(Vector3p& pos_neu_m)
{
