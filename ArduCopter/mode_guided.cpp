...
bool ModeGuided::do_user_takeoff_start(float takeoff_alt_cm)
{
    ...
    auto_takeoff.start(alt_target_cm * 0.01, alt_target_terrain);
...
bool ModeGuided::set_pos_neu_cm(const Vector3f& pos_neu_cm, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw, bool terrain_alt)
{
    ...
    guided_is_terrain_alt = terrain_alt;
    guided_vel_target_neu_cms.zero();
    guided_accel_target_neu_cmss.zero();
...
bool ModeGuided::set_destination(const Location& dest_loc, bool use_yaw, float yaw_rad, bool use_yaw_rate, float yaw_rate_rads, bool relative_yaw)
{
    ...
    guided_pos_target_neu_cm = pos_target_f.topostype();
    guided_is_terrain_alt = terrain_alt;
    guided_vel_target_neu_cms.zero();
    guided_accel_target_neu_cmss.zero();
    update_time_ms = millis();
...
void ModeGuided::pos_control_run()
{
    ...
    if (guided_is_terrain_alt && !wp_nav->get_terrain_offset_cm(terr_offset)) {
        // failure to set destination can only be because of missing terrain data
    ...
    if (guided_is_terrain_alt) {
        pos_offset_z_buffer = MIN(copter.wp_nav->get_terrain_margin_m() * 100.0, 0.5 * fabsF(guided_pos_target_neu_cm.z));
    }
    pos_control->input_pos_NEU_cm(guided_pos_target_neu_cm, terr_offset, pos_offset_z_buffer);
