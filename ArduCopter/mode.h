...
public:
    void run();
    void start(float complete_alt_m, bool terrain_alt);
    bool get_completion_pos(Vector3p& pos_neu_m);
...
protected:
    ...
    class _TakeOff {
    public:
        void start(float alt_m);
        void stop();
        void do_pilot_takeoff_ms(float& pilot_climb_rate_ms);
        bool triggered(float target_climb_rate_ms) const;
...
public:
    ...
    bool set_pos_neu_cm(const Vector3f& pos_neu_cm, bool use_yaw = false, float yaw_rad = 0.0, bool use_yaw_rate = false, float yaw_rate_rads = 0.0, bool yaw_relative = false, bool terrain_alt = false);
